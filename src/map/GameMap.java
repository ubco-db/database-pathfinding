...
//	private RegionSearchProblem abstractProblem;		// Only used for PRA*
	...
	{	// Loads a map in Vadim's format into data structure
		load(fileName);
		generator = new Random();		
	}
	...
	public SparseMask createCoverageMask(byte[][] coverage)
	{
		...
		{	for (int c=0; c < cols; c++)
			{
				if (!isWall(r, c))
				{	// Determine all cells that are reachable from this one via database entries						
	...
	{	HashMap<Integer,MapPoint> result = new HashMap<Integer,MapPoint>();
		...
		while (!currentSet.isEmpty())
		{
			...
			if (isValid(r-1, c) && !isWall(r-1, c))
				if (squares[r-1][c] != val)	// Above
					result.put(squares[r-1][c], new MapPoint(r-1,c));
			else
				currentSet.add(new MapPoint(r-1,c));
		
		if (isValid(r-1, c + 1) && !isWall(r-1, c+1))
			if (squares[r-1][c+1] != val) // Top right
				result.put(new Integer(squares[r-1][c+1]), new MapPoint(r-1,c+1));
			else
				currentSet.add(new MapPoint(r-1,c+1));
		
		if (isValid(r, c + 1) && !isWall(r, c + 1))
			if (squares[r][c + 1] != val) // Right
				result.put(new Integer(squares[r][c + 1]), new MapPoint(r,c+1));
			else
				currentSet.add(new MapPoint(r,c+1));
		
		if (isValid(r+1,c+1) && !isWall(r+1,c+1))
			if (squares[r+1][c+1] != val) // Bottom right
				result.put(new Integer(squares[r+1][c+1]), new MapPoint(r+1,c+1));
			else
				currentSet.add(new MapPoint(r+1,c+1));
		
		if (isValid(r+1,c) && !isWall(r+1, c))
			if (squares[r + 1][c] != val) // Bottom 
				result.put(new Integer(squares[r+1][c]), new MapPoint(r+1,c));
			else
				currentSet.add(new MapPoint(r+1,c));
		
		if (isValid(r + 1, c - 1) && !isWall(r + 1, c - 1))
			if (squares[r + 1][c - 1] != val) // Bottom left
				result.put(new Integer(squares[r + 1][c - 1]), new MapPoint(r+1,c-1));
			else
				currentSet.add(new MapPoint(r+1,c-1));
		
		if (isValid(r, c - 1) && !isWall(r, c - 1))
			if (squares[r][c - 1] != val) // Left
				result.put(new Integer(squares[r][c - 1]), new MapPoint(r,c-1));
			else
				currentSet.add(new MapPoint(r,c-1));
		
		if (isValid(r - 1, c - 1) && !isWall(r - 1, c - 1))
			if (squares[r - 1][c - 1] != val) // Top left
				result.put(new Integer(squares[r - 1][c - 1]), new MapPoint(r-1,c-1));
			else
				currentSet.add(new MapPoint(r-1,c-1));		
	}
	...
	public boolean isClique(Set<Integer> nodes)
	{	// Idea: To be a clique, everyone has to be a neighbour of everyone else.
		...
		while (it.hasNext())
		{
			...
			{			
				...
				if (i.intValue() != i2.intValue() && !isNeighbor(i, i2))
					return false;
			}
		}
		return true;
	...
	public HashMap<Integer,MapPoint> findAllCliqueNeighbors(int row, int col, int val, HashMap<Integer,Integer> statesUsed)
	{	// Idea: Find all the neighbours then determine if they are all connected (clique).
	...
	public GameMap computeBorder()
	{	// Identifies points in map that are along a transition border
		GameMap baseMap = this.copyMap();
		int borderPoints = 0;
							
		for (int r = 0; r < rows; r++)
		{	for (int c = 0; c < cols; c++)
			{	if (baseMap.squares[r][c] == EMPTY_CHAR)
				{	// Border point if any non-diagonal cell is a wall
	...
	public GameMap neighbourCondense()
	{	// Condenses a group into another group if it has an unique neighbor
		GameMap baseMap = this.copyMap();		
		...
		{	for (int c = 0; c < cols; c++)
			{	if (baseMap.squares[r][c] == EMPTY_CHAR)
				{	// Determine value of this cell in other map
			...
			{	if (i==1)
				...
				{	
					...
					{	int row = r+startRow;
						...
						if (isOpenCell(row, col) && !currentSet.isFound(id))
						{ 		// Mutual reachable check below													
   ...
   */
	...
	{	// Only computes the neighbor group ids for each group not the list of neighbor cells
	...
	public void addGroup(int id, GroupRecord group)
	{
		...
		else if (groupsArray.length-1 < id)
		{	// Resize array
			int size = id*2;			
	...
	public GameMap cliqueAbstract()
	{
		...
		for (int i = 0; i < rows; i++)
		{
			for (int j = 0; j < cols; j++)
			{
				...
				{
					if (this.squares[i][j] == ' ')
					{	// Base case for level 1 abstraction
						currentNum++;
						// Not assigned square
				...
				//		group.setGroupRepId(seedId);
					...
					{	// For higher levels of abstraction beyond 1
	...
	public GameMap coverAbstract(int numTrailHeads, SearchAbstractAlgorithm searchAlg)
	{
		...
		if (precompute)
		{
			...
			{	for (int c =0; c < this.cols; c++)
				{
					if (!this.isOpenCell(r, c))
						continue;		// Cell is a wall
			...
			for (int i=0; i < numTrailHeads; i++)
			{
				...
				{	for (int c =0; c < this.cols; c++)
					{
						...
						for (int a=0; a < grec.states.num(); a++)
							if (!coveredStates.get(grec.states.get(a)))
								newCount++;
				...
				if (bestCoverage == null)
					break;			// No more trailheads needed as everything covered
							
				currentNum++;
				System.out.println("Computed trail head: "+(i+1)+" Id: "+maxCoverageStateId+" Row: "+this.getRow(maxCoverageStateId)+" Col: "+this.getCol(maxCoverageStateId)
						+" New states covered: "+bestCoverCount+ " Group: "+currentNum);
				...
				{	int id = bestCoverage.states.get(a);
					if (coveredStates.get(bestCoverage.states.get(a)))
						continue;
		...
		{	// This version does not pre-compute coverages							 
			for (int i=0; i < numTrailHeads; i++)
			{
				bestCoverage = null;
				...
				System.out.println("Computed trail head: "+(i+1)+" Id: "+maxCoverageStateId+" Row: "+this.getRow(maxCoverageStateId)+" Col: "+this.getCol(maxCoverageStateId)
						+" New states covered: "+bestCoverCount);
...
//	}
	...
	private void addEdge(int fromRegion, int toRegion, int [][]edges)
	{
		...
		{	// Copy over existing array if not already there
			for (int i=0; i < edges[fromRegion].length; i++)
				if (edges[fromRegion][i] == toRegion)
	...
	{	
		...
		for (int i = 0; i < rows; i++)
		{
			...
			{							
				...
				{	// Lookup color used for this number
					col = (Color) colors.get(val);
					if (col == null)
					{	// TODO: How to pick color.  Picking a random color for now.
	...
	public int getZId(int row, int col)
	{	// Interleave bits of row and column.  
		int id = 0;
		int num = col;
		int count = 1;
		while (num > 0)
	...
	{		
		...
		int width = 7;
		if (this.cols > 1000 || this.rows > 1000)
			largeMap = true;
	...
	public void computeComplexity(DBStatsRecord stats)
	{	// Computes the complexity of a map according to a given map abstraction algorithm
	...
	{	// Returns the size of the map if use RLE to encode map
	...
	{				
		...
		if (currentSeedId == GameMap.EMPTY_CHAR)
			return true;		
	...
	public void computeCentroids()
	{
		...
		{	// Find centroid for each record
			...
			if (this.isWall(row, col) || squares[row][col] != rec.groupId)				
			{	// If centroid point is not in group or is a wall
