...
//	}
		...
		{						
			...
			for (int i=1; i < numLevels; i++)
			{
				...
				{				
					...
					if (done.get(neighborId))
						continue;	// Already processed this neighbors set
	...
	{				
		...
		{	count = 0;
			...
			for (int j=0; j < numGroups; j++)
				if (neighbor[i][j] >= 0)
					tmpNeighbors[i][count++] = neighbor[i][j];
		}
		...
		{	
			startGroup = groups.get(i+GameMap.START_NUM);
			if (startGroup.getSize() < SIZE_CUTOFF)
				continue;
			...
			for (j=0; j < numGroups; j++)
			{
				...
				if (goalGroup.getSize() < SIZE_CUTOFF)
					continue;
				...
				if (pathSize == 0)
					continue;		// No path between two states				
				...
				if (subgoals == null)
					...
					numSubgoals += subgoals.length+1;
				
				SubgoalDBRecord rec = new SubgoalDBRecord(count, startGroup.groupRepId, goalGroup.groupRepId, subgoals,
						(startGroup.groupId-GameMap.START_NUM+1)*10000+goalGroup.groupId-GameMap.START_NUM);	// TODO: This will probably need to be changed.
	...
	{			
		...
		while (count > 0)
		{ 	// Find lowest distance (linear search)
