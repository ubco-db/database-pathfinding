...
//					break;			
	...
	{				
		...
		{	int r = map.getRow(j);
			int c = map.getCol(j);
			if (!map.isWall(r, c))
			{	// Include counts if a wall
				numCounts++;
				totalCounts += startCounts[j];
				if (startCounts[j] < minCount)
					minCount = startCounts[j];
				if (startCounts[j] > maxCount)
					maxCount = startCounts[j];
			}			
		}
		...
		{	for (int j=0; j < fullProblemSize; j++)
			{
				...
				if (map.isWall(startId) || map.isWall(goalId))
					invalid++;
				else
				{
					if (coveredProblems.get(idx))
