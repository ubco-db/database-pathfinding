...
public class SubgoalDBExact extends SubgoalDB
{
	...
	{	
		...
		if (rec == null)
		{	
			if (startSeedId == goalSeedId)
			{	// This is possible as there is no guarantee that you can go from any two states in a region (need to direct through seed).
			...
			result.add(recordMatrix[startSeedId][goalSeedId]);
		return result;						
	}
	...
	{	
		...
		{	
			...
			return recordMatrix[startSeedId][goalSeedId];						
	}
	/**
	...
	private void generateIndexDB()
	{
		...
		{	
			...
			if (!distinctStates.containsKey(lastStateVal))
				distinctStates.put(lastStateVal, lastStateVal);
		}		
