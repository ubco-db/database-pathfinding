...
public class SubgoalSearch implements SearchAlgorithm
{
	...
	public ArrayList<SearchState> computePath(SearchState start, SearchState goal, StatsRecord stats)
	{
		...
		if (path != null)
		{	// System.out.println("Global goal is HC reachable from start.");
			subgoals.add(goal);
		...
		{				
			...
			currentIndex = -1;			
			if (OPTIMIZE_START_DB_PATH)
			{	// Try to hill-climb to first subgoal instead of database goal start as will improve optimality
				...
				else
				{
					...
					if (newPath != null)
					{	// Possible to hill climb to subgoal
						currentIndex = 1;
						startOptimizationDone = true;
						...
						if (currentIndex >= subgoalList.length)
						{	// Default is to go to goal of database record then we will hill climb from there.
							...
							if (OPTIMIZE_END_DB_PATH)
							{	// newPath = AStar.reachableHillClimbing(map, currentStart, goal, cutoff, stats);
								newPath = optCheckSearchAlg.computePath(currentStart, goal, stats);
								if (newPath != null)
								{	// From last subgoal spot can reach global goal using hill climbing.  That completes the path.
			...
			if (!startOptimizationDone && OPTIMIZE_CHECK)
			{	// Unable to hill climb to first subgoal - try to hill climb somewhere between start and first subgoal
			...
			if (!startOptimizationDone)
			{	// If no start optimization succeeds, go to record start
				currentGoal = new SearchState(currentRecord.getStartId());					
				subgoals.add(currentGoal);
			}
		}	
		...
		{											
			...
			{	// TODO: Not sure what to do in this case
				if (currentGoal == goal)
				{	// Special case to handle situation where did not guarantee HC-reachability in both directions
			...
			currentIndex++;
			if (subgoalList == null)
			{	if (currentGoal.id == currentRecord.getGoalId())
					currentGoal = goal;
				else
					currentGoal = new SearchState(currentRecord.getGoalId());		
			}
			else if (currentIndex == subgoalList.length)
			{
				...
				if (OPTIMIZE_END_DB_PATH)
				{	// System.out.println("End optimization check from: "+problem.idToString(currentStart.id)+" to "+problem.idToString(goal.id));
					newPath = optCheckSearchAlg.computePath(currentStart, goal, stats);				
					if (newPath != null)
					{	// From last subgoal spot can reach global goal using hill climbing.  That completes the path.
						// Trying to go to the global goal from the last subgoal may not work but will often be shorter than going to the database goal point then our desired goal.
					...
					{						
						if (OPTIMIZE_CHECK)
						{	// Unable to hill climb to goal from last subgoal - try to hill climb somewhere between last subgoal and end to global goal
