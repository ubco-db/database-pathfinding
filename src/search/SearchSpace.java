...
public class SearchSpace
{
	...
	public SearchSpace cliqueAbstract(DBStatsRecord dbstat)
	{
		...
		{	// No abstraction (or grouping) has been performed yet on this search problem.  Perform first level of abstraction
			// Iterate through the ResultSet building cliques of base states
			
			problem.initIterator();
			while (problem.nextState(currentState))
			{
				if (abstractSpace.states[currentState.id] != 0)
					continue;
				...
				{	int addNodeId = neighbors.get(i);
					if (abstractSpace.states[addNodeId] != 0)
						continue;
		...
		{	// Perform level 2 or higher abstraction
			// IDEA: Iterate through the groups.  Find all neighbors for all states in the group.  Any of those are then reachable.
			boolean inClique;
		
			Iterator<Integer> groupIt = this.groups.keySet().iterator();
			while (groupIt.hasNext())
			{	GroupRecord g = groups.get(groupIt.next());
				...
				if (groupsInClique.size() == 1)
				{	// Merge with one of its other groups
					...
					{	int neighborId = it.next();
						...
						if (neighborGroupId == g.groupId || neighborGroup == null)
							continue;	
	...
	{				
	    ...
	    {	g2.setColor(Color.WHITE);
	    	...
	    	{	
	    		...
	    		row = s.id / cols;
	    		if (scale > 1)
	    			g2.fillRect(row*scale+scale/3, col*scale+scale/3, scale/3, scale/3);
	    		else
	    			g2.fillRect(row, col, 1, 1);
	    	}
	    }
	...
	{	ArrayList<SearchState> seeds = new ArrayList<SearchState>(groups.size());
		Iterator<GroupRecord> it = groups.values().iterator();
	
		while (it.hasNext())
		{	GroupRecord g = it.next();
