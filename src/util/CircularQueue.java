...
public class CircularQueue {
    ...
    public int remove() {
        if (front != -1) {  // Must be an element in the queue
            int val = data[front];
     ...
     */
    public int next() {
        if (iteratorLoc == -1) return 0;                   // Queue is empty

        if (iteratorLoc == maxSize) iteratorLoc = 0;        // Wrapped around in circular queue
    ...
    */
    public int size() {
        if (front == -1) return 0;
        else if (rear < front) return maxSize - front + rear + 1;         // rear pointer is less than front
        else return rear - front + 1;
    }

    public boolean insert(int val) {
        if ((front == 0 && rear == maxSize - 1) || (rear + 1 == front)) return false;   // Queue is full
